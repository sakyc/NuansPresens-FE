  ...
  const fetchMessages = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Jika backend kirim [{}, {}] maka pakai data
        // Jika backend kirim { responses: [] } maka pakai data.responses
        setMessages(Array.isArray(data) ? data : (data.responses || []));
      }
    } catch (error) {
          ...
          messages.map((m) => {
            ...
            return (
              <div key={m.id} className={`flex ${ownMsg ? "justify-end" : "justify-start"}`}>
                <div
                  className={`max-w-[80%] rounded-xl px-4 py-3 ${ownMsg
